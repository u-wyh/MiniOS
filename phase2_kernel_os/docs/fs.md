...
但 Phase2 最终态下，“文件”已经不只来自内核静态只读数据，而是分成两类：
...
- 阶段二前半段，文件系统主要是“内置只读文件表”
- 阶段二后半段，又补上了教学版 RAMFS、fd 写入、追加写、stdin/stdout 重定向到文件

因此当前 Phase2 的文件系统定位更准确地说是：

- 没有真实磁盘
- 没有持久化
- 但已经具备“只读内置文件 + 可写 RAMFS 文件”的教学版文件系统雏形
...
## 20. Task71：输入文件 + pipe buffer + 屏幕输出

Task71 当前把 Task67 的文件 stdin 重定向和 Task69 的 pipe buffer 组合起来，形成：
...
1. 左侧进程先通过 `SYS_READ(fd=0)` 从输入文件读取
2. 左侧进程的 `SYS_WRITE` 不输出到屏幕，而是写入 pipe buffer
3. 左侧结束后，右侧进程再运行
4. 右侧进程通过 `SYS_READ(fd=0)` 从 pipe buffer 读取
5. 右侧进程的 `SYS_WRITE` 继续正常输出到屏幕

### 输入文件、pipe buffer 与文件系统的关系

1. 输入源可以是内置只读文件
2. 输入源也可以是 RAMFS 文件
3. pipe buffer 仍然不是文件系统对象
4. pipe buffer 不会出现在 `ls`
5. pipe buffer 不能被 `cat /path` 访问
