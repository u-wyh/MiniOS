...
6. `mini_pipeline` / shell 多级管道

但还没有完整 Unix/Linux 那套 file object、引用计数、阻塞队列、signal 和进程组语义。
...
4. pipe fd 当前只是共享 `pipe_id`，不是完整共享 open file object
5. 阻塞等待仍然是教学版 busy-wait / retry
...
2. 关闭 pipe fd 时，只更新对应 `pipe_id` 那个 pipe object 的端状态，不主动清空已有数据
3. 因为当前没有引用计数，这仍然不是完整 POSIX 生命周期模型
4. 子进程退出后，不应错误清空父进程稍后还要读取的 pipe object 数据
...
3. 这里描述的是 Task90 当时的顺序模型；最终态在 Task96 之后已经推进到“多子进程并存 + 教学版等待”的最小并发模型
...
这里描述的是 Task94 当时的顺序 pipeline；最终态已在 Task96 之后推进到最小并发模型。
...
1. 父进程只负责搭线和回收
2. 左右两个子进程并存
3. 数据通过对应 `pipe_id` 的 pipe object 在二者之间流动
